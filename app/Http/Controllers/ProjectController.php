...
class ProjectController extends Controller
{
    ...
    public function getSecData(Request $request) 
    {
        ...
        $query = Project::where('status', 0)->orderBy('id','desc');
    ...
    public function create(Request $request)
    {
        ...
        try {
            ...
            return response()->json(['success' => 'SDG Project Created Successfully'], 201);
        } catch (\Exception $e) {
            return response()->json(['error' => $e->getMessage()], 500);
        }
    }

    public function getProjectById($row_id)
    {
        try {
            $project = Project::find($row_id);
            $sdg_criteria = SdgCriteria::all();
            if ($project) {
                return view('modals.view_project_request', compact('project','sdg_criteria'));
            }
        } catch (\Exception $e) {
            return response()->json(['error' => $e->getMessage()], 500);
        }   
    ...
    public function getProcessedProjectById($row_id)
    {
        try {
            $project = Project::find($row_id);
            if ($project) {
                return view('modals.view_project_processed', compact('project'));
            }
        } catch (\Exception $e) {
            return response()->json(['error' => $e->getMessage()], 500);
        }   
    ...
    {
        $data = Project::find($row_id);
        if($data) {
            $data->status = 2;
            $data->sdg_approved = $sdg_approved;
            $data->save();
            return response()->json(['success' => 'Project approved successfully'], 204);
        }
        return response()->json(['error' => 'Failed to reject project'], 204);
    }
